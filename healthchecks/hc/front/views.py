...
def monitoring_infrastructure(request: AuthenticatedHttpRequest) -> HttpResponse:
    queries = {
        ...
        "disk": '100 * (1 - (node_filesystem_avail_bytes{mountpoint="/",fstype!="rootfs"} / node_filesystem_size_bytes{mountpoint="/",fstype!="rootfs"}))',
        "load": "node_load1",
        "nginx_requests": "rate(nginx_http_requests_total[5m])",
        "nginx_active": "nginx_connections_active",
    }
    return JsonResponse(
        {name: _prom_query_range(query) for name, query in queries.items()}
    )
...
def monitoring_metrics(request: HttpRequest) -> HttpResponse:
    ...
    project_codes = [item["code"] for item in _monitoring_config()]
    projects = {
        str(project.code): project
        for project in Project.objects.filter(code__in=project_codes)
    }
    config_by_code = {item["code"]: item for item in _monitoring_config()}

    def esc(value: str) -> str:
        return value.replace("\\", "\\\\").replace('"', '\\"').replace("\n", "\\n")

    lines = [
        "# HELP hc_monitoring_check_up Whether the monitored check is up.\n",
        "# TYPE hc_monitoring_check_up gauge\n",
    ]
    ...
    for code, project in projects.items():
        ...
        for check in checks:
            ...
            if check.last_ping:
                lines.append(
                    f"hc_monitoring_check_last_ping_timestamp_seconds{{{labels}}} {int(check.last_ping.timestamp())}\n"
                )
            if check.last_duration:
                lines.append(
                    f"hc_monitoring_check_last_duration_seconds{{{labels}}} {check.last_duration.total_seconds():.3f}\n"
                )

        for status in ("up", "down", "grace", "new", "paused"):
            lines.append(
                f'hc_monitoring_checks_total{{project="{esc(project_name)}",status="{status}"}} {counts[status]}\n'
            )
